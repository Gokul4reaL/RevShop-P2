...
//    }
	...
	/*
	 * @Test public void testUpdatePaymentStatus_Success() { Long paymentId = 1L;
	 * PaymentStatus status = PaymentStatus.SUCCESS; PaymentDetails
	 * updatedPaymentDetails = new PaymentDetails();
	 * updatedPaymentDetails.setId(paymentId);
	 * updatedPaymentDetails.setStatus(status);
	 * 
	 * when(paymentService.updatePaymentStatus(paymentId,
	 * status)).thenReturn(updatedPaymentDetails);
	 * 
	 * ResponseEntity<PaymentDetails> response =
	 * paymentController.updatePaymentStatus(paymentId, status);
	 * 
	 * assertEquals(HttpStatus.OK, response.getStatusCode());
	 * assertNotNull(response.getBody()); assertEquals(status,
	...
	/*
	 * @Test public void testGetPaymentDetails_Success() { Long paymentId = 1L;
	 * PaymentDetails paymentDetails = new PaymentDetails();
	 * paymentDetails.setId(paymentId);
	 * 
	 * when(paymentService.getPaymentDetails(paymentId)).thenReturn(Optional.of(
	 * paymentDetails));
	 * 
	 * ResponseEntity<PaymentDetails> response =
	 * paymentController.getPaymentDetails(paymentId);
	 * 
	 * assertEquals(HttpStatus.OK, response.getStatusCode());
	 * assertNotNull(response.getBody()); assertEquals(paymentId,
