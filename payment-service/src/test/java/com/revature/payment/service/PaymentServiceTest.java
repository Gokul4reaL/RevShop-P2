...
public class PaymentServiceTest {
	 ...
	 * PaymentStatus status = PaymentStatus.SUCCESS; PaymentDetails paymentDetails =
	 * new PaymentDetails(); paymentDetails.setId(paymentId);
	 * paymentDetails.setStatus(PaymentStatus.INITIATED);
	 * 
	 * when(paymentRepository.findById(paymentId)).thenReturn(Optional.of(
	 * paymentDetails));
	 * when(paymentRepository.save(any(PaymentDetails.class))).thenReturn(
	 * paymentDetails);
	 * 
	 * PaymentDetails result = paymentService.updatePaymentStatus(paymentId,
	 * status);
	 * 
	 * assertNotNull(result); assertEquals(PaymentStatus.SUCCESS,
	 * result.getStatus()); verify(paymentRepository,
	 * times(1)).save(paymentDetails); }
	 */
	 ...
	 * paymentDetails));
	 * 
	 * Optional<PaymentDetails> result =
	 * paymentService.getPaymentDetails(paymentId);
	 * 
	 * assertTrue(result.isPresent()); assertEquals(paymentId,
	 * result.get().getId()); }
	 */
