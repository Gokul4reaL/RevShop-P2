...
public class ProductServiceTest {
	...
	/*
	 * @Test public void testGetAllProducts_Success() { List<ProductResponse>
	 * mockProducts = Arrays.asList( new ProductResponse(), new ProductResponse() );
	 * 
	 ...
	 * doReturn(Flux.fromIterable(mockProducts)).when(responseSpec).bodyToFlux(
	 * ProductResponse.class);
	 * 
	 * List<ProductResponse> result = productService.getAllProducts();
	 * 
	 * assertEquals(2, result.size()); assertEquals("Product1",
	 * result.get(0).getName()); verify(webClientBuilder, times(1)).build(); }
	 */
	...
	/*
	 * @Test public void testGetProductById_Success() { Long productId = 1L;
	 * ProductResponse mockProduct = new ProductResponse();
	 * 
	 * doReturn(webClient).when(webClientBuilder).build();
	 * doReturn(requestHeadersUriSpec).when(webClient).get();
	 * doReturn(requestHeadersSpec).when(requestHeadersUriSpec).uri(
	 * "http://localhost:8087/products/" + productId);
	 * doReturn(responseSpec).when(requestHeadersSpec).retrieve();
	 * doReturn(Mono.just(mockProduct)).when(responseSpec).bodyToMono(
	 * ProductResponse.class);
	 * 
	 * ProductResponse result = productService.getProductById(productId);
	 * 
	 * assertNotNull(result); assertEquals("Product1", result.getName());
	...
	/*
	 * @Test public void testAddProduct_Success() { ProductRequest productRequest =
	 * new ProductRequest(); productRequest.setName("NewProduct");
	 * 
	 * // Mocking the WebClient call chain for POST
	 * doReturn(webClient).when(webClientBuilder).build();
	 * doReturn(requestBodySpec).when(webClient).post(); ((UriSpec<?>)
	 * doReturn(requestHeadersSpec).when(requestBodySpec)).uri(
	 * "http://localhost:8087/products"); ((RequestBodySpec)
	 * doReturn(requestHeadersSpec).when(requestHeadersSpec)).bodyValue(
	 * productRequest); doReturn(responseSpec).when(requestHeadersSpec).retrieve();
	 * doReturn(Mono.just("Product added successfully")).when(responseSpec).
	 * bodyToMono(String.class);
	 * 
	 * String result = productService.addProduct(productRequest);
	 * 
	 * assertEquals("Product added successfully", result); verify(webClientBuilder,
	 * times(1)).build(); }
	 */
