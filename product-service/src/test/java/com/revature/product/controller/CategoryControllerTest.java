...
public class CategoryControllerTest {
	 ...
	 * 
	 * when(categoryService.getCategoryById(categoryId)).thenReturn(Optional.of(
	 * category));
	 * 
	 ...
	 * assertNotNull(response.getBody()); assertEquals("Test Category",
	...
	/*
	 * @Test public void testGetAllCategories_Success() { List<Category>
	 * categoryList = new ArrayList<>(); Category category = new Category();
	 * category.setId(1L); category.setName("Test Category");
	 ...
	 * assertFalse(response.isEmpty()); assertEquals(1, response.size());
	 ...
	 * assertEquals(HttpStatus.OK, response.getStatusCode());
	 * assertNotNull(response.getBody()); assertEquals("New Category",
	 * response.getBody().getName()); }
	 */
